...
df <- df %>%
  mutate(
    ...
    # Convertible/2-in-1 flag
    is_convertible = ifelse(
      "is_2in1" %in% names(df) && !is.null(is_2in1),
      is_2in1,
      grepl("2-in-1|2in1|convertible|touchscreen|yoga|flex|spin|spectre x360", 
    ...
    brand_display = case_when(
      brand == "NIMO" ~ "Budget Brand",
      brand == "" | is.na(brand) ~ "Other",
      TRUE ~ brand
    ),
...
# 3. GET UNIQUE VALUES
brands <- sort(unique(df$brand_display[df$brand_display != "Other" & df$brand_display != "Budget Brand"]))
              ...
              fluidRow(
                box(
                  ...
                  awesomeCheckboxGroup("explorer_features", "Special Features:",
                                       ...
                                       status = "primary"),
                  br(),
                  actionBttn("explorer_apply_filters", "Apply Filters",
                             icon = icon("filter"),
                             style = "gradient",
                             color = "primary",
                             block = TRUE),
...
# 5. SERVER LOGIC - UPDATED WITH IMPROVED RECOMMENDATIONS
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    ...
    if (input$sidebar_brand != "All") {
      data <- data %>% filter(brand_display == input$sidebar_brand)
    }
  ...
  # === 2. EXPLORER FUNCTIONALITY (ONLY ONCE) ===
  explorer_trigger <- reactiveVal(0)
  
  observeEvent(input$explorer_apply_filters, {
    explorer_trigger(explorer_trigger() + 1)
  ...
  explorer_data <- reactive({
    ...
    # Apply filters only if trigger > 0 (button has been clicked)
    if (explorer_trigger() > 0) {
      
      # Apply retailer filter
      if (!is.null(input$explorer_retailer)) {
      ...
      # Apply brand filter
      if (!is.null(input$explorer_brand)) {
        if (!"All" %in% input$explorer_brand) {
          data <- data %>% filter(brand_display %in% input$explorer_brand)
        }
      }
      
      # Apply price filter
      if (!is.null(input$explorer_price)) {
        data <- data %>%
      ...
      if (!is.null(input$explorer_processor)) {
        if (!"All" %in% input$explorer_processor) {
          data <- data %>% filter(processor_tier %in% input$explorer_processor)
        }
      }
      
      # Apply feature filters
      if (!is.null(input$explorer_features)) {
        if ("Gaming" %in% input$explorer_features) {
          data <- data %>% filter(is_gaming_flag == TRUE)
        }
        if ("2-in-1" %in% input$explorer_features) {
          data <- data %>% filter(is_convertible == TRUE)
        }
        if ("Student Friendly" %in% input$explorer_features) {
          data <- data %>% filter(is_student_friendly == TRUE)
        }
      }
    }
  ...
  output$analysis_scatter <- renderPlotly({
    ...
    p <- plot_ly(data, x = ~get(x_var), y = ~extracted_price,
                 ...
                 text = ~paste("Brand:", brand_display,
 ...
 # =============== processor_analysis =============
  ...
  output$explorer_table <- renderDT({
    ...
    if (explorer_trigger() == 0) {
      message_df <- data.frame(
        Instruction = "Click 'Apply Filters' to see data",
        Steps = "1. Select filters from the left panel\n2. Click 'Apply Filters' button\n3. Filtered data will appear here"
      )
      
      return(datatable(message_df,
                       options = list(dom = 't', pageLength = 5),
                       rownames = FALSE))
    }
    
    # Show actual data if filters have been applied
    if (nrow(data) == 0) {
      message_df <- data.frame(
        Message = "No data found with current filters",
        Suggestion = "Try widening your price range or selecting 'All' for retailers/brands"
      )
    ...
    display_data <- data %>%
      select(
        ...
        Gaming = is_gaming_flag,
        `2-in-1` = is_convertible
      )
    ...
    datatable(display_data,
      ...
      formatRound("Rating", 2)
  })
  
  # === 4. DEBUG OUTPUT ===
  output$explorer_debug <- renderPrint({
    cat("=== EXPLORER DEBUG ===\n")
    cat("explorer_trigger value:", explorer_trigger(), "\n")
    cat("explorer_data() is NULL?", is.null(explorer_data()), "\n")
    
    if (!is.null(explorer_data())) {
      data <- explorer_data()
    ...
    cat("\nFilter inputs:\n")
    cat("explorer_retailer:", paste(input$explorer_retailer, collapse = ", "), "\n")
    cat("explorer_brand:", paste(input$explorer_brand, collapse = ", "), "\n")
    cat("explorer_price:", input$explorer_price[1], "-", input$explorer_price[2], "\n")
  })
  ...
  output$brand_market_share <- renderPlotly({
    data <- filtered_data() %>%
      filter(brand_display != "Other") %>%
      count(brand_display) %>%
      mutate(percentage = n / sum(n) * 100) %>%
      arrange(desc(n)) %>%
      head(10)
    
    plot_ly(data, labels = ~brand_display, values = ~percentage,
            type = "pie",
...
# ==================retailer_value_comparison===================
  ...
  output$gaming_premium_analysis <- renderPlotly({
    ...
    plot_ly(price_summary,
            ...
            type = "bar",
            marker = list(
              color = c(professional_colors$secondary, professional_colors$danger),
              line = list(color = professional_colors$dark, width = 1)  # SINGLE value
            ),
      ...
      layout(
        ...
        annotations = list(
          list(
            ...
            yref = "y",
            showarrow = FALSE,
            font = list(size = 14, color = professional_colors$danger, weight = "bold"),
            bgcolor = "white",
  ...
  # 2. SMART RECOMMENDATIONS - IMPROVED
  ...
  recommendations <- eventReactive(input$find_laptops, {
    ...
    # Apply use case specific filters with intelligent adjustments
    if (input$use_case == "Gaming") {
      data <- data %>% 
        filter(is_gaming_flag == TRUE) %>%
        filter(ram_category %in% c("16GB", "32GB", "64GB+")) %>%
        mutate(case_weight = 1.5)  # Boost gaming laptops
      
    } else if (input$use_case == "Student") {
      data <- data %>% 
        filter(is_student_friendly == TRUE | extracted_price <= 800) %>%
        filter(ram_category %in% c("8GB", "16GB", "32GB", "64GB+")) %>%
        mutate(case_weight = 1.2)  # Slightly boost student-friendly
      
    } else if (input$use_case == "2-in-1/Convertible") {
      data <- data %>% 
        filter(is_convertible == TRUE) %>%
        mutate(case_weight = 1.3)  # Boost convertibles
    } else {
      data <- data %>% mutate(case_weight = 1.0)
    }
    
    # Apply budget with intelligent range
    # If user selects $1000, we look for laptops within 80%-120% of budget
    ...
    data <- data %>%
      ...
      filter(rating_numeric >= input$min_rating) %>%
      filter(ram_gb >= input$min_ram)
    
    if (input$brand_pref != "No Preference") {
      data <- data %>% filter(brand_display == input$brand_pref)
    }
    
    if (nrow(data) == 0) {
      # If no results, expand the search
      data <- df %>%
        filter(extracted_price >= input$budget * 0.7,
               extracted_price <= input$budget * 1.5) %>%
        filter(rating_numeric >= input$min_rating * 0.9) %>%
        filter(ram_gb >= input$min_ram * 0.5)
    }
    ...
    data <- data %>%
      mutate(
        ...
        ram_score = ram_gb / max(ram_gb, na.rm = TRUE),
        storage_score = ifelse(!is.na(storage_gb), storage_gb / max(storage_gb, na.rm = TRUE), 0.5),
        rating_score = rating_numeric / 5,
        
        # Processor score based on tier
        ...
        # User-weighted score
        final_score = (
          (price_score * (input$weight_value/100)) +
            (ram_score * (input$weight_performance/100)) +
            (storage_score * (input$weight_features/100)) +
            (rating_score * 0.3) +
            (processor_score * (input$weight_performance/100))
        ) * case_weight,
        
        # Value score (price vs features)
        value_performance = (ram_gb + ifelse(!is.na(storage_gb), storage_gb/100, 0)) / (extracted_price/100)
      ) %>%
      arrange(desc(final_score)) %>%
      head(15)  # Return top 15 for better selection
  ...
  output$recommendation_details <- renderUI({
    ...
    if (nrow(recs) > 0) {
      ...
      tagList(
        ...
        tags$div(
          style = "background-color: #f8f9fa; padding: 15px; border-radius: 5px;",
          tags$h5(tags$strong(top_rec$brand_display, ": ", top_rec$title)),
  ...
  output$recommendation_comparison_chart <- renderPlotly({
    ...
    plot_ly(recs, x = ~brand_display, y = ~final_score * 100,
            type = "bar",
            marker = list(color = color_scale,
                          line = list(color = professional_colors$dark, width = 1)),
  ...
  output$recommendation_radar <- renderPlotly({
    ...
    recs <- recommendations() %>% head(3)
    
    # Create radar chart data
    radar_data <- data.frame(
      metric = rep(c("Price Value", "RAM", "Rating", "Processor", "Storage"), each = 3),
      value = c(
        recs$final_score * 100,
        recs$ram_gb / max(recs$ram_gb) * 100,
        recs$rating_numeric / 5 * 100,
        case_when(
          recs$processor_tier == "High-End" ~ 100,
          recs$processor_tier == "Performance" ~ 80,
          recs$processor_tier == "Mid-Range" ~ 60,
          recs$processor_tier == "Entry-Level" ~ 40,
          TRUE ~ 30
        ),
        recs$storage_gb / max(recs$storage_gb, na.rm = TRUE) * 100
      ),
      brand = rep(recs$brand_display, 5)
    )
    
    plot_ly(radar_data, type = 'scatterpolar', mode = 'lines+markers',
            r = ~value, theta = ~metric, color = ~brand,
            colors = color_scale[1:3],
            fill = 'toself',
            fillcolor = ~alpha(color, 0.3)) %>%
      layout(polar = list(radialaxis = list(visible = TRUE, range = c(0, 100))),
             showlegend = TRUE,
             title = "Top 3 Recommendations Comparison",
             plot_bgcolor = professional_colors$light,
             paper_bgcolor = professional_colors$light)
  })
  
  # 3. VALUE ANALYSIS - FIXED
  output$value_analysis_plot <- renderPlotly({
    ...
    plot_ly(top_value, x = ~brand_display, y = ~value_ratio,
            type = "bar",
            marker = list(color = color_scale,
                          line = list(color = professional_colors$dark, width = 1)),
            text = ~paste("Price: $", extracted_price,
